        ...
        extends DefaultTraversalVisitor<R, C>
{
    ...
    public Set<String> generateMaterializedViewCandidate()
    {
        ...
        for (QualifiedName baseTable : tableNames) {
            List<String> materializedViewList = baseTableToMaterializedViewMap.getOrDefault(baseTable.getSuffix(), new ArrayList<>());
            if (visitAtLeastOneBaseTable) {
                materializedViewCandidate.retainAll(materializedViewList);
            }
            else {
                materializedViewCandidate.addAll(materializedViewList);
            }

            visitAtLeastOneBaseTable = true;
